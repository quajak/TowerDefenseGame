...
namespace TrySFML2
{
    internal class Cannon : Tower
    {
        ...
        public Stat ExplosionSize = new Stat(40);
        public int ExplosionDamage = 1;

        public Cannon(float x, float y, bool buy = false) : base(x, y, new RectangleShape(new Vector2f(20, 20)), _cost, "Cannon", "Shoot bullets which explode", 100, 2, 500)
        {
            if (buy)
                Program.Money -= _cost;
            ...
            CustomUpgrade pierce = new CustomUpgrade(null, 5, "Pierce I", "Each shell has 1 more pierce", (t) => (t as Cannon).ExtraPierce += 1)
            {
                Unlocks =
                {
                     new CustomUpgrade(null, 12, "Pierce II", "Each shell has 2 more pierce", (t) => (t as Cannon).ExtraPierce += 2)
                     {
                         Unlocks =
                         {
                              new CustomUpgrade(null, 20, "Pierce III", "Each shell has 16 more pierce", (t) => (t as Cannon).ExtraPierce += 3)
                         }
                     }
                }
            };
                ...
                lock (Program.toChange) lock (Program.enemies)
                    {
                        ...
                        if (list.Count != 0)
                        {
                            ...
                            Bullet bullet = new Bullet(position.X, position.Y, vX * scale, vY * scale, 600, amount.Value, amount.Value,
                                1, new Vector2f(8, 8), (p) => Program.toChange.Add(new Explosion(p.X, p.Y, ExplosionSize.Value, ExplosionDamage)));
