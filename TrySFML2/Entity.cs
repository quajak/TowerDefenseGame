    ...
    class Entity
    {
        public IEnumerable<Vector2f> Points
        {
            get {
                ...
                float x = 0;
                float y = rectangleShape.Size.Y;// rectangleShape.Size.Y;
                ...
                lock (Program.toChange) lock (Program.enemies)
                    {
                        var possible = from enemy in Program.enemies
                                       where Program.Distance(this, enemy) < 1000
                                       select enemy;
                        var list = possible.OrderBy(x => Program.Distance(this, x)).Take(10).OrderBy(e => (e as Enemy).DistanceToGoal).ToList();
                        if (list.Count != 0)
                        {
                            Entity item = list[0];
                            //Generate bullet
    ...
    class MachineGun : Tower
    {
        ...
        public override Shape Update(double timeDiff)
        {
            ...
            if(attackTime < 0)
            {
                ...
                {
                        var possible = from enemy in Program.enemies
                                       where Program.Distance(this, enemy) < 200
                                       select enemy;
                        var list = possible.OrderBy(x => Program.Distance(this, x)).Take(1).ToList();
                        if(list.Count != 0)
                        {
                            Entity item = list[0];
                            //Generate bullet
                            ...
                            float scale = Program.Scale(vX, vY, 1300);
    ...
    class Enemy : Entity
    {
        public float DistanceToGoal
        {
            get
            {
                ...
                {
                    return Program.Distance(this, Program.mainBase);
                }
                return 100f;
            }
        }
        ...
        public Enemy(int aX, int aY) : base(aX, aY, new RectangleShape(new Vector2f(5, 5)))
        {
            ...
            float s = Program.Scale(vX, vY, speed);
